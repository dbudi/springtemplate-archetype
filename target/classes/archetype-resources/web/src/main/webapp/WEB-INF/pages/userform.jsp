...
<script type="text/javascript">var msgDelConfirm =
   "<fmt:message key="delete.confirm"><fmt:param value="${symbol_dollar}{delObject}"/></fmt:message>";
</script>

<div class="col-sm-2">
    <h2><fmt:message key="userProfile.heading"/></h2>
    <c:choose>
        <c:when test="${symbol_dollar}{param.from == 'list'}">
...
<div class="col-sm-7">
    ...
    <form:form commandName="user" method="post" action="userform" id="userForm" autocomplete="off"
        ...
        <input type="hidden" name="from" value="<c:out value="${symbol_dollar}{param.from}"/>"/>
        ...
        <div class="form-group${symbol_dollar}{(not empty status.errorMessage) ? ' has-error' : ''}">
            ...
            <c:if test="${symbol_dollar}{pageContext.request.remoteUser == user.username}">
        ...
        <div>
            <legend class="accordion-heading">
                <a data-toggle="collapse" href="${symbol_pound}collapse-address"><fmt:message key="user.address.address"/></a>
            </legend>
            <div id="collapse-address" class="accordion-body collapse">
                ...
                <div class="form-group">
                    ...
                    <appfuse:country name="address.country" prompt="" default="${symbol_dollar}{user.address.country}"/>
                </div>
            </div>
        </div>
<c:choose>
    <c:when test="${symbol_dollar}{param.from == 'list' or param.method == 'Add'}">
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <select id="userRoles" name="userRoles" multiple="true" class="form-control">
                ...
                <option value="${symbol_dollar}{role.value}" ${symbol_dollar}{fn:contains(user.roles, role.label) ? 'selected' : ''}>${symbol_dollar}{role.label}</option>
                </c:forEach>
            </select>
        </div>
    </c:when>
    <c:when test="${symbol_dollar}{not empty user.username}">
        <div class="form-group">
            <label class="control-label"><fmt:message key="user.roles"/>:</label>
            <div class="readonly">
                <c:forEach var="role" items="${symbol_dollar}{user.roleList}" varStatus="status">
                    <c:out value="${symbol_dollar}{role.label}"/><c:if test="${symbol_dollar}{!status.last}">,</c:if>
                    <input type="hidden" name="userRoles" value="<c:out value="${symbol_dollar}{role.label}"/>"/>
                </c:forEach>
            </div>
        ...
        <div class="form-group">
            ...
            <c:if test="${symbol_dollar}{param.from == 'list' and param.method != 'Add'}">
