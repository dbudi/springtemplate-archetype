...
<div class="col-sm-3">
    ...
    <c:choose>
        <c:when test="${symbol_dollar}{not empty token}">
...
<div class="col-sm-5">
	<form method="post" id="updatePassword" action="<c:url value='/updatePassword'/>" class="well" autocomplete="off">
        ...
        <div class="form-group">
            ...
            <input type="text" name="username" class="form-control" id="username" value="<c:out value="${symbol_dollar}{username}" escapeXml="true"/>" required>
        </div>

	    <c:choose>
	    	<c:when test="${symbol_dollar}{not empty token}">
			    <input type="hidden" name="token" value="<c:out value="${symbol_dollar}{token}" escapeXml="true" />"/>
	    	</c:when>
	    	<c:otherwise>
		        <div class="form-group">
