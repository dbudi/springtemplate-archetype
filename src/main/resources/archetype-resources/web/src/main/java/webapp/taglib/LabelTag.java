...
public class LabelTag extends TagSupport {
    ...
    public int doStartTag() throws JspException {
        ...
        if ((message == null) || "".equals(message.trim())) {
            ...
            label.append("<label for=${symbol_escape}"").append(fieldName).append("${symbol_escape}"");

            if (cssClass != null) {
                label.append(" class=${symbol_escape}"").append(cssClass);
                if (validationError) {
                    label.append(" ").append(cssErrorClass);
                }
            }

            label.append("${symbol_escape}">").append(message);
            label.append((requiredField) ? " <span class=${symbol_escape}"required${symbol_escape}">*</span>" : "");
            label.append((colon) ? ":" : "");
            label.append("</label>");
        }
