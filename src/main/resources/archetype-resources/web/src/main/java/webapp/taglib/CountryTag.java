...
#set( $symbol_dollar = '$' )
#set( $symbol_escape = '\' )
package ${package}.webapp.taglib;

import ${package}.model.LabelValue;
 ...
 * @version ${symbol_dollar}Revision: 1.6 ${symbol_dollar} ${symbol_dollar}Date: 2006/07/15 11:57:20 ${symbol_dollar}
 *
...
public class CountryTag extends TagSupport {
    ...
    public int doStartTag() throws JspException {
        ...
        if (scope != null) {
            ...
            sb.append("<select name=${symbol_escape}"" + name + "${symbol_escape}" id=${symbol_escape}"" + name + "${symbol_escape}" class=${symbol_escape}"form-control${symbol_escape}">${symbol_escape}n");

            if (prompt != null) {
                sb.append("    <option value=${symbol_escape}"${symbol_escape}" selected=${symbol_escape}"selected${symbol_escape}">");
                sb.append(eval.evalString("prompt", prompt) + "</option>${symbol_escape}n");
            }

            for (Iterator i = countries.iterator(); i.hasNext();) {
                LabelValue country = (LabelValue) i.next();
                sb.append("    <option value=${symbol_escape}"" + country.getValue() + "${symbol_escape}"");

                if ((selected != null) && selected.equals(country.getValue())) {
                    sb.append(" selected=${symbol_escape}"selected${symbol_escape}"");
                }

                sb.append(">" + country.getLabel() + "</option>${symbol_escape}n");
            }
     ...
     *
     * @see javax.servlet.jsp.tagext.Tag${symbol_pound}release()
     */
    public void release() {
        super.release();
    }
